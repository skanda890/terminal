...
try
{
    ...
    if (til::starts_with_insensitive_ascii(string, L"rgb:"))
    {
        ...
        size_t i = 0;
        for (const auto& part : til::split_iterator{ string.substr(4), L'/' })
        {
            const auto val = til::parse_unsigned<unsigned int>(part, 16);
            if (!val || i >= std::size(parameters))
            {
                return {};
            }
            parameters[i++] = *val;
...
til::small_vector<std::wstring_view, 4> Utils::SplitString(const std::wstring_view wstr, const wchar_t delimiter) noexcept
try
{
    til::small_vector<std::wstring_view, 4> result;

    for (const auto& part : til::split_iterator{ wstr, delimiter })
    {
